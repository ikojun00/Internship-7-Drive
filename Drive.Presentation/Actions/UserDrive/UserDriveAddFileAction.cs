...
{
    public class UserDriveAddFileAction : ICommandAction
    {
        ...
        public UserDriveAddFileAction(DriveRepository driveRepository)
        {
        ...
        public void Open()
        {
            Console.Write($"Jeste li sigurni da želite stvoriti datoteku '{UserContext.CurrentName}'? (da/ne): ");
            var confirmation = Console.ReadLine()?.ToLower();

            if (confirmation != "da")
            {
                Console.WriteLine("Stvaranje datoteke je otkazano.");
                return;
            }
            ...
            var (result, message) = _driveRepository.CreateFile(
